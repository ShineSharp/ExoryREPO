                    ...
                    Variables.Menu.Item($"{Variables.MainMenuName}.espell.whitelist.{Targets.Target.ChampionName.ToLower()}").IsActive())
                {
                    for (int i = 1; i < 10; i++)
                    {
                        if ((Targets.Target.Position + Vector3.Normalize(Targets.Target.ServerPosition - ObjectManager.Player.Position) * i * 43).IsWall() &&
                            (Variables.E.GetSPrediction(Targets.Target).UnitPosition.To3D2() + Vector3.Normalize(Targets.Target.ServerPosition - ObjectManager.Player.Position) * i * 43).IsWall() &&
                            (Variables.E.GetSPrediction(Targets.Target).UnitPosition.To3D2() + Vector3.Normalize(Targets.Target.ServerPosition - ObjectManager.Player.Position) * i * 44).IsWall())
                        {
